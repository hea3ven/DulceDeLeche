...
public class MetalFurnaceRecipes {
	...
	public void addRecipe(int tier, ItemStack input, ItemStack output) {
		recipes.put(Pair.of(input, (ItemStack) null), new MetalFurnaceRecipe(tier, input, null, output));
	}

	public void addRecipe(int tier, String oreInput1, int sizeInput1, String oreInput2, int sizeInput2,
			ItemStack output) {
	...
	public void addMetalRecipe(int tier, Metal metal) {
		for (ItemStack input1 : OreDictionary.getOres(metal.getOreName())) {
			input1 = input1.copy();
			if (metal == Metal.GOLD)
				MetalFurnaceRecipes.instance().addRecipe(tier, input1, new ItemStack(Items.gold_nugget));
			else
				MetalFurnaceRecipes.instance()
						.addRecipe(tier, input1, ProxyCommonDulceDeLeche.nugget.createStack(metal, 3));
		}
		for (ItemStack input1 : OreDictionary.getOres(metal.getIngotName())) {
			input1 = input1.copy();
			input1.stackSize = 9;
			if (metal == Metal.IRON)
				MetalFurnaceRecipes.instance().addRecipe(tier, input1, new ItemStack(Blocks.iron_block));
			else if (metal == Metal.GOLD)
				MetalFurnaceRecipes.instance().addRecipe(tier, input1, new ItemStack(Blocks.gold_block));
			else
				MetalFurnaceRecipes.instance()
						.addRecipe(tier, input1, ProxyCommonDulceDeLeche.metalBlock.createStack(metal));
		}
		for (ItemStack input1 : OreDictionary.getOres(metal.getNuggetName())) {
			input1 = input1.copy();
			input1.stackSize = 9;
			if (metal == Metal.IRON)
				MetalFurnaceRecipes.instance().addRecipe(tier, input1, new ItemStack(Items.iron_ingot));
			else if (metal == Metal.GOLD)
				MetalFurnaceRecipes.instance().addRecipe(tier, input1, new ItemStack(Items.gold_ingot));
			else
				MetalFurnaceRecipes.instance()
						.addRecipe(tier, input1, ProxyCommonDulceDeLeche.ingot.createStack(metal));
		}
	}

	public void addAlloyRecipe(int tier, Metal inputMetal1, int sizeInput1, Metal inputMetal2, int sizeInput2,
			Metal outputMetal, int sizeOutput) {
		...
		for (ItemStack input1 : OreDictionary.getOres(outputMetal.getIngotName())) {
			...
			input1.stackSize = 9;
			MetalFurnaceRecipes.instance()
					.addRecipe(tier, input1, ProxyCommonDulceDeLeche.metalBlock.createStack(outputMetal));
		}
		for (ItemStack input1 : OreDictionary.getOres(outputMetal.getNuggetName())) {
			input1 = input1.copy();
			input1.stackSize = 9;
			MetalFurnaceRecipes.instance()
					.addRecipe(tier, input1, ProxyCommonDulceDeLeche.ingot.createStack(outputMetal));
		}
	}
	...
	public ItemStack getSmeltingResult(ItemStack stack1, ItemStack stack2) {
		MetalFurnaceRecipe recipe = getRecipe(stack1, stack2, true);
	...
	public MetalFurnaceRecipe getRecipe(ItemStack stack1, ItemStack stack2, boolean matchSize) {
		...
		for (Entry<Pair<ItemStack, ItemStack>, MetalFurnaceRecipe> entry : recipes.entrySet()) {
			...
			if (matchSize && (itemsMatch(input1, stack1) && input1.stackSize > stack1.stackSize) ||
					(itemsMatch(input1, stack2) && input1.stackSize > stack2.stackSize))
				continue;

			if (matchSize && input2 != null &&
					((itemsMatch(input2, stack1) && input2.stackSize > stack1.stackSize) ||
							(itemsMatch(input2, stack2) && input2.stackSize > stack2.stackSize)))
				continue;
	...
	private boolean itemsMatch(ItemStack stack1, ItemStack stack2) {
		return stack1 == null && stack2 == null ||
				ItemStack.areItemsEqual(stack1, stack2) && stack1.getMetadata() == stack2.getMetadata();
	}
